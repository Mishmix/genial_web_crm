...
(function initThumbnailCarousel() {
    ...
    var autoAdvanceTimer = setInterval(function() {
        nextSlide();
    }, 4000);
    ...
    // Resume auto-advance after touch ends (with delay)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Channels expand/collapse functionality - incremental show
    const channelsGrid = document.getElementById('channels-grid');
    const expandBtn = document.getElementById('channels-expand-btn');
    
    if (channelsGrid && expandBtn) {
        const allChannelCards = channelsGrid.querySelectorAll('.channel-card');
        const totalChannels = allChannelCards.length;
        
        // Determine initial and increment counts based on screen size
        const isMobile = () => window.innerWidth <= 768;
        const getInitialCount = () => isMobile() ? 4 : 6;
        const getIncrementCount = () => isMobile() ? 4 : 6;
        
        let visibleCount = getInitialCount();
        let isFullyExpanded = false;
        
        // Initialize: show first N channels, hide rest
        function initializeChannels() {
            const initialCount = getInitialCount();
            visibleCount = initialCount;
            isFullyExpanded = false;
            
            allChannelCards.forEach((card, index) => {
                card.classList.remove('channel-hidden', 'channel-visible', 'channel-revealing');
                if (index < initialCount) {
                    card.classList.add('channel-visible');
                } else {
                    card.classList.add('channel-hidden');
                }
            });
        ...
        // Show more channels with animation
        function showMoreChannels() {
            const increment = getIncrementCount();
            const startIndex = visibleCount;
            const endIndex = Math.min(visibleCount + increment, totalChannels);
            
            // Animate new channels
            for (let i = startIndex; i < endIndex; i++) {
                const card = allChannelCards[i];
                card.classList.remove('channel-hidden');
                card.classList.add('channel-visible', 'channel-revealing');
                card.style.animationDelay = `${(i - startIndex) * 0.08}s`;
                
                // Remove animation class after animation completes
                setTimeout(() => {
                    card.classList.remove('channel-revealing');
            ...
            // Change to collapse button if all channels are visible
        ...
        // Collapse channels back to initial state
        function collapseChannels() {
            const initialCount = getInitialCount();
            
            // Hide extra channels with animation
            for (let i = totalChannels - 1; i >= initialCount; i--) {
                const card = allChannelCards[i];
                card.classList.remove('channel-visible', 'channel-revealing');
                card.classList.add('channel-hidden');
            }
...
(function initPortfolioShowcase() {
    ...
    function getFilteredItems() {
        ...
        const filtered = allItems.filter(item => item.dataset.category === currentFilter);
        return filtered;
    }
    ...
    function initializeVisibility() {
        ...
        // Update button visibility - only hide the button itself, not the container
    ...
    function collapsePortfolio() {
        ...
        // First animate out current visible items
        const currentVisible = Array.from(visibleSet);
        ...
        // After fade out, show new random items
        setTimeout(() => {
    ...
    showMoreBtn.addEventListener('click', () => {
        ...
        // Show items in DOM order up to newVisibleCount with staggered animation
        let animIndex = 0;
        filtered.forEach((item, index) => {
            if (index < newVisibleCount) {
                ...
                if (wasHidden && index >= visibleCount) {
                    // New item - animate it with stagger
                    item.style.display = '';
                    item.style.opacity = '0';
                    item.style.transform = 'translateY(20px) scale(0.95)';
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Animate out current visible items
            const currentVisible = Array.from(visibleSet);
            currentVisible.forEach((item) => {
                item.style.transition = 'opacity 0.25s ease, transform 0.25s ease';
                item.style.opacity = '0';
...
(function() {
    ...
    const getInitialShow = () => isMobile() ? 3 : 4;
    const getShowMore = () => isMobile() ? 3 : 4;
    
    let visibleCount = getInitialShow();
    ...
    casesExpandBtn.addEventListener('click', () => {
        ...
        if (visibleCount >= cases.length) {
            ...
            visibleCount = Math.min(visibleCount + getShowMore(), cases.length);
